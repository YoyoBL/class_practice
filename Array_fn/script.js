// 1. sort the following array
// ["buy milk", "go to school", "study", "build project", "right song", "draw"]

// 2. with the following arrays answere the following questions:
// 	- do they contain only numbers?
// 	- do they contain only truthy values?
// 	- do they have an elemnt with the value 3?
// 	- do they have an element with a value less than 0?
// 	- find the first string value with 5 letters
// 	- find all string values with 5 letters
// 	- find only numeric values and multiply by 3
// [23,5,6,22,"a", true,3,5,7,3, "hello", "daniel"]
// [3,4,5,6,0, 7]
// [true, {}, []]
...
function onlyMachos(arr) {
   console.log(arr.filter((x) => x.gender === "male"));
}

// onlyMachos(crazyList);

function sortByName(arr, key) {
   const copyCrazyList = [...arr];
   console.log(copyCrazyList.sort((a, b) => (a[key] > b[key] ? 1 : -1)));
}

// sortByName(crazyList, "name");
// console.log(crazyList);

function filterByAge(arr) {
   console.log(
      arr.filter((x) => x.age >= 18).sort((a, b) => (a.age > b.age ? 1 : -1))
   );
}

// filterByAge(crazyList);

function greeenEyes(arr) {
   console.log(arr.some((x) => x.eyeColor === "green"));
}

function userNameArrayCreator(arr) {
   return arr.map((x) => x.name);
}

// let arr = userNameArrayCreator(crazyList);
let copy;
...
function originalAndNewStuff(arr) {
   deepCopy(arr);
   copy.filter((x) => {
      x.numberOfFriends = x.friends.length;
      x.numberOfTags = x.tags.length;
   });
   console.log(copy);
}

// originalAndNewStuff(crazyList);
// console.log(crazyList);

let $text = document.getElementById("text");

function ageFilter(arr) {
   let newCopy = arr.filter((x) => x.age > 22);
   newCopy = newCopy.forEach((x) => {
      $text.innerHTML += `Welcome, ${x.name} (${x.age})<br>`;
   });
}

ageFilter(crazyList);

// 3. from the following array do as follows:
// 	- find only males
// 	- sort all users by name
// 	- sort all users by age
// 	- sort all users by isActive
// 	- find only users over 18
// 	- are some of the users with green eyes?
// 	- create array only with the users names
